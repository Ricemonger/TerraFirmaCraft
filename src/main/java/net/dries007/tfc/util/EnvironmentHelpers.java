...
public final class EnvironmentHelpers {
    ...
    public static void tickChunk(ServerLevel level, LevelChunk chunk, ProfilerFiller profiler) {
        ...
        final BlockPos randomPos = new BlockPos(lcgPos.getX(), Mth.nextInt(level.getRandom(), -64, 320), lcgPos.getZ());
        ...
        meltSnowBlock(level, randomPos, temperature);
    ...
    private static void meltSnowBlock(Level level, BlockPos pos, float temperature) {
        final BlockState state = level.getBlockState(pos);
        if (state.getBlock() == Blocks.SNOW_BLOCK && (temperature > OverworldClimateModel.SNOW_FREEZE_TEMPERATURE || level.getBrightness(LightLayer.BLOCK, pos) > 11)) {
